...
names(dados)
dados %<>% mutate(dados, `Pública` = `Pública`/1000)
dados <- dados %>% mutate(across(`Pública`, ~round(.x,1)))
...
objeto_0 <- dados %>%
  ...
  select(Ano,Pública) %>% filter(Ano>2000) %>%
  arrange(Ano) %>%
...
data_serie <- paste('[',gsub(' ',',',
                             paste(paste(as.vector(objeto_0[[1]]$Pública)),
                                   collapse = ' ')),']',sep = '')
